...
export class AlarmPredictor {
  ...
  public predictForSensor(sensorId: string): SensorPrediction | null {
    try {
      ...
      const windowMs = this.config.historyWindowMinutes * 60 * 1000
      const cutoffTime = now - windowMs
      const rawHistory = this.history.get(sensorId) || []
      const history = rawHistory.filter(h => h.timestamp >= cutoffTime)
      ...
      const xPrediction = (predictionTimestamp - history[0].timestamp) / 1000
  ...
  private linearRegression(points: HistoryPoint[]): {
    slope: number
    intercept: number
    rSquared: number
  } | null {
...
export function createAlarmPredictor(config?: Partial<PredictionConfig>): AlarmPredictor {
  return new AlarmPredictor(config)
}
